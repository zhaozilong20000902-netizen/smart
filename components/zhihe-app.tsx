      ...
      <main className="main">
        ...
        {view === "scan" && <section className="page scan-page">
          ...
          {result && <div id="report" className="report-section">
            ...
            {result.mode !== "model" && <div className="demo-banner"><Info /><span><strong>{result.modelWarning || "当前使用演示结果。"}</strong> {result.mode === "demo" ? "配置 MODEL_API_URL、MODEL_API_KEY、MODEL_NAME 后会自动调用真实模型。" : "本次报告已保持可用，教师可继续复核；稍后重新检查可再次调用真实模型。"}</span></div>}
